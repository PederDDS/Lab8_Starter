describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    if(innerText['_remoteObject'].value != '20'){
      console.log(innerText['_remoteObject'].value)
      test_param = false;
    }
    expect(test_param).toBe(true);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const cartCount = await page.$('#cart-count');
    const innerCart = await cartCount.getProperty('innerText');

    let test_param = true;
      if (innerCart['_remoteObject'].value != '0'){
        test_param = false;
      }
    expect(test_param).toBe(true);
  }, 10000);
